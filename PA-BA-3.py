...
def bland_altman_prepare(x, y, mode="Absolute difference"): 
 ...
 #   todf = slovar_todf.sort_values(by='mean')
    ...
    #    mm = todf['mean']
        ...
        #xlabel = r'$ln(\sqrt{XY})$'
        #ylabel = r'$ln(Y/X)$'
        xlabel = r'$(ln(M_1)+ln(M_2))/2$'
        ylabel = r'$ln(M_1)-Ln(M_2)$'       
...
#                                                                       'Regression for relative difference (%)',
 #                                                                      'Regression in log-scale'])
    ...
    report_mode = st.sidebar.radio("Выбери вариант отчёта", ["BA+PB",
                                                             ...
                                                             "BA(rank vs abs diff)+BA(rank vs relative diff)+PB+residuals",
                                                             "BA+Deming",
                                                             "BA+Mountain+Deming"])
    st.sidebar.subheader('⚙️ Дополнительные настройки') 
    okrugl = st.sidebar.number_input('Округление для БА. Число знаков', value=4, min_value=0,max_value=4, step=1)
    ...
    if st.sidebar.button("Запустить анализ"):
        ...
        # поиграем с округлениями    
        def BA_table_show(okrugl):
            ba_table = pd.DataFrame({ 
                    " ": ["Mean Bias", "LoA lower", "LoA upper"], 
                    "Value": [round(ba_res["md"], okrugl), round(ba_res["loa_lower"],okrugl), round(ba_res["loa_upper"],okrugl)],
                    "95% CI" : [f'{round(ba_res["ci_md"][0],okrugl)}  —  {round(ba_res["ci_md"][1],okrugl)}',
                                 f'{round(ba_res["ci_lower"][0],okrugl)}  —  {round(ba_res["ci_lower"][1],okrugl)}',
                                 f'{round(ba_res["ci_upper"][0],okrugl)}  —  {round(ba_res["ci_upper"][1],okrugl)}']})
        ...
        for m, (b1, b2) in enumerate(st.session_state.pairs): 
            ...
            if report_mode == "BA(rank vs abs diff)+BA(rank vs relative diff)+PB+residuals":
                with c1:
                    ba_res = bland_altman_analysis(x, y, mode= 'Rank (X+Y)/2 vs (X-Y)')
                    BA_show()
                with c_reg:
                    if advanced_ba:
                        BA_reg_show(ba_mode='Rank (X+Y)/2 vs (X-Y)')
                    else:
                        pass
                with c21:
                    BA_table_show(okrugl)
                    BA_sum(ba_summary_data)
                with c2:
                    ba_res = bland_altman_analysis(x, y, mode="Rank (X+Y)/2 vs 100*(X-Y)/Mean")
                    BA_show()
                with c_reg2:
                    if advanced_ba:
                        BA_reg_show(ba_mode="Rank (X+Y)/2 vs 100*(X-Y)/Mean")
                    else:
                        pass
                with c22:
                    BA_table_show(okrugl)
