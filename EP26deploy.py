...
with tab0:
    ...
    if bb[0] !=0:
        tablo = {"Концентрация": [cc[0]],
             ...
             "Swrl": [round(bb[0],2)] if cr04 == 'Swlr' else [round(bb[0]*cc[0]/100,2)],
    ...
    for i in range(1,slevel):
        if bb[i] !=0:
            ...
            tablo["Swrl"].append(round(bb[i],2) if cr04 == 'Swlr' else round(bb[i]*cc[i]/100,2))
    ...
    for i in range (0, slevel):
        ...
        maxres = np.max(res)
        sdres = tdf1['Swrl'][i]
        tablo4["Диапазон концентраций"].append(f'{round(minres-2*sdres, 1)} - {round(maxres+2*sdres, 1)}')
