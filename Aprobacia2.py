...
def plot_confusion_matrix(a, b, c, d):
    ...
    #im = ax.pcolormesh(matrix, cmap=new_cmap, edgecolors = None)
...
def plot_3x3_agreement(table):
    ...
    ax.set_xticks(np.arange(0, table.shape[1], 1))
    ax.set_yticks(np.arange(0, table.shape[0], 1))
...
if uploaded_file:
    ...
    if radio_type == 'Количественные в бинарные':
        ...
        push = st.sidebar.button("Запуск вычислений ▶️")
        if push:
    ...
    if radio_type == 'Количественные в бинарные + неопределенность':
        ...
        " коэффициентов согласованности от cut-off в диапазоне от 0 до введенного значения",value=3)
         ...
         push_u = st.sidebar.button("Запуск вычислений ▶️")
         if push_u:
